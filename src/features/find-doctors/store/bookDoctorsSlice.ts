...
import {bookDoctorAPI, cancelBookDoctor, getBookDoctorsAPI} from "./bookDoctorsAPI";
import {TBookDoctorsState} from "../types";
...
export const bookDoctorsSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(bookDoctorAPI.fulfilled,
            (state, { payload }) => {
        ...
        builder.addCase(bookDoctorAPI.rejected,
            (state, { payload }) => {
        ...
        builder.addCase(getBookDoctorsAPI.fulfilled,
            (state, { payload }) => {
                state.items = [...payload];
                state.status = EStatus.succeeded;
            });
        builder.addCase(getBookDoctorsAPI.rejected,
            (state, { payload }) => {
                state.status = EStatus.failed;
            });
        ...
        builder.addCase(cancelBookDoctor.fulfilled,
            (state, { payload }) => {
                console.log('payload', payload)
                state.items = [...payload];
                state.status = EStatus.succeeded;
            });
        builder.addCase(cancelBookDoctor.rejected,
            (state, { payload }) => {
                state.status = EStatus.failed;
            });
    },
...
export const selectBookDoctorStatus = (state: RootState) => state.bookDoctors.status;
...
export const selectBookDoctorFetchStatus = (state: RootState) => state.bookDoctors.status
