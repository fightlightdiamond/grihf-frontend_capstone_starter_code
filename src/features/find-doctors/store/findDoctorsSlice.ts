...
import type { TFindDoctorsState } from '../types';
...
export const findDoctorsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(findDoctorsAPI.fulfilled, (state, { payload }) => {
      state.items = payload;
      state.status = EStatus.idle;
    });
    builder.addCase(findDoctorsAPI.rejected, (state) => {
...
export const selectFindDoctorFetchStatus = (state: RootState) =>
  state.findDoctors.status;
