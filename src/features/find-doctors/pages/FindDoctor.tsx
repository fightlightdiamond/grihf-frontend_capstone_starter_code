...
import { selectFindDoctors } from '../store/findDoctorsSlice';
...
const FindDoctor: React.FC = () => {
  ...
  function onCancel(doctorId: number) {
    dispatch(
      cancelBookDoctor({ userId: authState?.id as number, doctorId }),
    ).unwrap();
  }

  return (
    <section className={'container mx-auto flex flex-col gap-6 relative'}>
      {hadSearch ? (
        <div className={'flex gap-4 justify-center'}>
          <Select
            ...
            id="specialty"
            required
            aria-placeholder={'Search doctors by specialty'}
          >
            <option value="">Search doctors by specialty</option>
            {Object.values(DoctorSpecialty).map((doctor) => (
              <option key={doctor}>{doctor}</option>
            ))}
          </Select>
          <Button onClick={onFindDoctor} type="submit">
            Find
          </Button>
        </div>
      ) : (
      ...
      <div className={'text-center'}>
        <h2 className={'text-2xl font-bold'}>
          {findDoctorsSelector.length} doctors available in
        </h2>
          ...
          return (
            ...
            >
              <div className={'flex flex-col justify-center gap-2 text-center'}>
                ...
                {!bookDoctorSelectors.find(
                  (bookDoctor) => bookDoctor.doctorId === doctor.userId,
                ) ? (
                  <Button onClick={() => onBooking(doctor)} type="submit">
                    Book Appointment No Booking Fee
                  </Button>
                ) : (
                  <Button
                    color={'red'}
                    onClick={() => onCancel(doctor.id)}
                    type="submit"
                  >
